...
export default function EmailsTab() {
  ...
  return (
    <div>
      <h2 className="font-display" style={{ fontSize: 22, fontWeight: 600, margin: '0 0 4px' }}>Email tracker</h2>
      <p style={{ color: 'var(--slate)', fontSize: 13.5, margin: '0 0 22px' }}>
        Anything sitting 7+ days with no reply is flagged for resend.
      </p>
      ...
      {rows.length === 0 ? (
        ...
        <table>
          <thead><tr><th>Name</th><th>Company / Role</th><th>Email</th><th>Sent</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            {rows.map(r => {
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    {r.status === 'replied'
                      ...
                      : d >= 7 ? <span className="badge badge-warn">Needs resend</span>
                  ...
                  <td style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <button className="btn-ghost" onClick={() => resend(r.id)}>Resend today</button>
